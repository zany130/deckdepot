...
| SYSTEM-SCOPE-FLATPAK | PASS_WITH_LIMITATIONS | 2026-09-16 PluginLoader itself is `auth_admin` for Flatpak system actions (no custom polkit, no root plugin). 2026-09-17 **SESSION_BRIDGE_PASS**: Gaming Mode PluginLoader child used `systemd-run --user` helper; `pkcheck` of `org.freedesktop.Flatpak.app-install` = **yes**; disposable `org.kde.kcharselect` system install/remove verified. Production now routes **system Flatpak mutations only** through that bridge (`session_bridge.py`). User Flatpak and AppMan stay on existing paths. Runtime capability probe is non-mutating; failure degrades system rows to read-only. DeckDepot does not create remotes or edit `/etc`. | system mutations via session bridge; capability-gated |
| SESSION-BRIDGE | PASS | 2026-09-17 Desktop Plasma probe plus Gaming Mode proof. Backend pid 2193, gamescope session 4. Helper under `user@1000.service` is polkit `yes` for install/update/uninstall. Evidence: `~/homebrew/data/DeckDepot/session-bridge-gaming.json`, `spikes/results/session-bridge-from-plugin.json`. Production integration uses transient `systemd-run --user --collect` only; no persistent user units. | system Flatpak mutations |
...
No release-required gate may remain `PENDING`. `GAMING-MODE-LAUNCH` remains `FAIL_USE_FALLBACK` (no plugin launch). `SYSTEM-SCOPE-FLATPAK` is `PASS_WITH_LIMITATIONS` via the session bridge.
...
| Check | Result |
|---|---|
| Ledger gates | None remain `PENDING`. Closed fallback unchanged: `GAMING-MODE-LAUNCH`. `SYSTEM-SCOPE-FLATPAK` is capability-gated via the session bridge. |
...
- **System-scoped Flatpak remote modification** (creating/changing host remotes or `/etc` polkit). Install/update/uninstall of **existing** system remotes is implemented via the verified session bridge (`SESSION_BRIDGE_PASS`). DeckDepot still does not add system remotes, install custom polkit, or set the Decky `root` flag.
